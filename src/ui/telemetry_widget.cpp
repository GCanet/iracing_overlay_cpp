...
void TelemetryWidget::renderABSIndicator(float width, float height) {
    ...
    dl->PathArcTo(c, arcR2, static_cast<float>(M_PI * 0.7),  static_cast<float>(M_PI * 1.3),  16);
    dl->PathStroke(ringCol, false, thick);

    // Right arcs ))
    dl->PathArcTo(c, arcR,  static_cast<float>(-M_PI * 0.35), static_cast<float>(M_PI * 0.35), 16);
    dl->PathStroke(ringCol, false, thick);
    dl->PathArcTo(c, arcR2, static_cast<float>(-M_PI * 0.3),  static_cast<float>(M_PI * 0.3),  16);
    dl->PathStroke(ringCol, false, thick);
...
void TelemetryWidget::renderInputTrace(float width, float height) {
    ...
    float step = width / static_cast<float>(n - 1);
